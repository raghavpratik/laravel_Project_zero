...
<head>
    ...
    <title>Pawsome Registration - Join the Pack!</title>
    <!-- Tailwind CSS CDN -->
...
<body>
    ...
    <div class="login-container">
        ...
        <form action="{{ route('account.processRegister') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="name" name="name" placeholder="Your Name" value="{{ old('name') }}"
                       class="shadow-sm appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:ring-emerald-300 focus:border-emerald-300 transition duration-300 ease-in-out @error('name') error-border @enderror">
                @error('name')
            ...
            <div class="mb-4">
                ...
                <input type="password" id="password" name="password" placeholder="••••••••"
                       class="shadow-sm appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:ring-emerald-300 focus:border-emerald-300 transition duration-300 ease-in-out @error('password') error-border @enderror">
                @error('password')
                    <p class="text-red-500 text-xs text-left mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-6">
                <label for="password_confirmation" class="block text-left text-gray-700 text-sm font-medium mb-2">Confirm Password</label>
        ...
        <p class="mt-6 text-gray-600 text-sm">
            ...
            <a href="{{ route('login') }}" class="font-semibold text-emerald-600 hover:text-emerald-800 transition duration-200 ease-in-out">Log In</a>
        </p>
    </div>
    ...
    <script>
        ...
        function initThreeJS() {
            ...
            const pawMaterial = new THREE.MeshPhongMaterial({ color: 0x8d6e63, shininess: 20 });
            const mainPad = new THREE.SphereGeometry(0.8, 32, 32);
            mainPad.applyMatrix4(new THREE.Matrix4().makeScale(1, 0.6, 1));
            ...
            toeGeometry.applyMatrix4(new THREE.Matrix4().makeScale(1, 0.8, 1));
        ...
        function animate() {
            ...
            if (bone) {
                ...
                bone.position.y = 2 + Math.sin(Date.now() * 0.001) * 0.5;
            }
            if (pawGroup) {
                pawGroup.rotation.y += 0.003;
                pawGroup.rotation.z += 0.003;
                pawGroup.position.x = 3 + Math.cos(Date.now() * 0.0015) * 0.5;
            }
